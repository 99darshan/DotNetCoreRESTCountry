...
namespace DotNetRestCountries
{
	public static class RestCountry
	{
		...
		private static List<Country> _GetCountriesListResponse(string requestUrl)
		{
			...
			try
			{
				...
				if (res.StatusCode == HttpStatusCode.OK)
				{
					...
					var token = JToken.Parse(resBody);
					return _Deserialize(resBody);
				}
				else
				{
		...
		private static List<Country> _Deserialize(string response)
		{
			var token = JToken.Parse(response);

			if (token is JArray)
			{
				List<Country> countries = JsonConvert.DeserializeObject<List<Country>>(response, new JsonSerializerSettings()
				{
			...
			{
				Country country = JsonConvert.DeserializeObject<Country>(response, new JsonSerializerSettings()
				{
					NullValueHandling = NullValueHandling.Ignore
				});
